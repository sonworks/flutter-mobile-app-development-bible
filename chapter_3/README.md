...
Flutterのウィジェットを利用してアプリケーションを構築する場合、基本ウィジェットツリー構造となる。

#### パフォーマンスの問題と対策
ツリー構造が持つ一般的な問題は、パフォーマンス低下に繋がりやすい構造（データ更新時に横断的な処理を実行する必要がある、など）のことである。

Flutterアプリケーションでは、ウィジェットツリーで全てを構築するため、buile メソッドの返り値であるウィジェットの階層を可能な限り薄くするとパフォーマンスが向上する。
>ウィジェットツリーのデータ構造では、データ更新時の深索に伴う計算量が _O(n)_ になるため、階層が深くなればなるほど、横断的な処理に時間を要してしまう。

##### 3.5.2.2 build メソッドが返すネスト構造の一例
```
Scaffold
├──── AppBar
│  └──── Text
...
...
```
>サンプルアプリケーションの単純なコードでも深さと幅はこの程度で、パフォーマンスの悪化を感じた時は、まずネスト構造を把握することから始める

##### InheritedWidget

ウィジェットツリーのパフォーマンス問題を解決するため必要となるWidgetとして、部分ツリーを再構築したり、素早く上位ツリーをたどったりするなど、処理効率をよくする場合に効果が期待できるWidget。

特徴
* 下位ツリーからの低い参照コスト。つまり、下位ツリーからInheritedWidget( _O(1)_ )に直接アクセスが可能
* 下位ツリーの特定ウィジェットのいへの変更通知が可能
